...
if [[ "$2" == "matmult" ]]; then
	...
	for size in ${elements[@]} ; 
		...
		set style line 2 lc rgb 'black' lt 1 lw 2 pt 7 ps 1.0
		...
		plot '$group.tmp' using 1:2 with linespoints ls 1 title "multMatRowVet-OPT", \
		'$group.tmp' using 1:3 with linespoints ls 2 title "multMatRowVet-OLD"
	EOF
		...
		plot '$group.tmp' using 1:4 with linespoints ls 1 title "multMatColVet-OPT", \
		'$group.tmp' using 1:5 with linespoints ls 2 title "multMatColVet-OLD"
	EOF
	...
	do
		...
		set output "$group-multMatRowVet.png"
		plot '$group.tmp' using 1:2 with linespoints ls 1 title "DP-OPT", \
		'$group.tmp' using 1:3 with linespoints ls 2 title "AVX DP-OPT", \
		'$group.tmp' using 1:4 with linespoints ls 3 title "DP-OLD", \
		'$group.tmp' using 1:5 with linespoints ls 4 title "AVX DP-OLD"
	EOF
		...
		set output "$group-multMatColVet.png"
		plot '$group.tmp' using 1:6 with linespoints ls 1 title "DP-OPT", \
		'$group.tmp' using 1:7 with linespoints ls 2 title "AVX DP-OPT", \
		'$group.tmp' using 1:8 with linespoints ls 3 title "DP-OLD", \
		'$group.tmp' using 1:9 with linespoints ls 4 title "AVX DP-OLD", \
	EOF
	done
...
#TESTE DE PERFORMANCE AUTOMATIZADO PARA O PROGRAMA: SistemasLineares
	...
	for size in ${elements[@]} ; 
		...
		plot '$group.tmp' using 1:2 with linespoints ls 1 title "gaussJacobi-OPT", \
		'$group.tmp' using 1:3 with linespoints ls 2 title "gaussJacobi-OLD"
	EOF
		...
		plot '$group.tmp' using 1:4 with linespoints ls 1 title "gaussSeidel-OPT", \
		'$group.tmp' using 1:5 with linespoints ls 2 title "gaussSeidel-OLD"
	EOF
