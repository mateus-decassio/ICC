...
if [[ "$2" == "matmult" ]]; then
	...
	for size in ${elements[@]} ; 
		...
		plot '$group.tmp' using 1:2 with linespoints ls 1 title "multMatRowVet"
	EOF
		...
		plot '$group.tmp' using 1:3 with linespoints ls 1 title "multMatColVet"
	EOF
...
#TESTE DE PERFORMANCE AUTOMATIZADO PARA O PROGRAMA: SistemasLineares
	...
	for size in ${elements[@]} ; 
		...
		plot '$group.tmp' using 1:2 with linespoints ls 1 title "gaussJacobi"
	EOF
		...
		plot '$group.tmp' using 1:3 with linespoints ls 1 title "gaussSeidel"
	EOF
